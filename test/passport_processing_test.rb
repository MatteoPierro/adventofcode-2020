...
class PassportProcessingTest < Minitest::Test
  def test_valid_passport
    ...
    assert PassportProcessing::Validator::SIMPLE_VALIDATOR.call(valid_passport)
  end

  def test_invalid_passport
    invalid_passport = 'pid:860033327 eyr:2020 hcl:#fffffd byr:1937 iyr:2017 cid:147 hgt:183cm'
    assert_equal PassportProcessing::Validator::SIMPLE_VALIDATOR.call(invalid_passport), false
  end
  ...
  def test_passport_with_invalid_birth_year
    ...
    assert_equal false, PassportProcessing::Validator::COMPLETE_VALIDATOR.call(invalid_passport)
  end
  ...
  def test_second_puzzle_solution
    assert_equal 188, PassportProcessing.count_valid_passports(
      validator: PassportProcessing::Validator::COMPLETE_VALIDATOR
    )
  end

  def test_count_valid_passport_complete_validator
    passport_filepath = './test/passport_file.txt'
    assert_equal 2, PassportProcessing.count_valid_passports(
  ...
  def test_count_all_invalid_passport_complete_validator
    ...
    assert_equal 0, PassportProcessing.count_valid_passports(
      passport_filepath: passport_filepath,
      validator: PassportProcessing::Validator::COMPLETE_VALIDATOR
    )
  end
  ...
  def test_count_all_valid_passport_complete_validator
    ...
    assert_equal 4, PassportProcessing.count_valid_passports(
      passport_filepath: passport_filepath,
      validator: PassportProcessing::Validator::COMPLETE_VALIDATOR
    )
  end
  ...
  def test_height_without_scale
    assert_equal false, PassportProcessing::Validator.validate_height('66')
  end

  def test_hair_color
    assert_equal false, PassportProcessing::Validator.validate_hair_color('#1')
    assert PassportProcessing::Validator.validate_hair_color('#123456')
    assert_equal false, PassportProcessing::Validator.validate_hair_color('#1234567')
  end
end
